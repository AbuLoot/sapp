...
@section('content')
  ...
  <h2 class="page-header">Отчет по сотрудникам</h2>

  @include('components.alerts')

  <form action="/{{ $lang }}/pos/report-workers" method="get">
    {!! csrf_field() !!}
    <div class="row">
      <div class="col-md-3">
        ...
        <table class="table table-striped">
          <thead>
            <tr>
              <th>ID</th>
              <th>Имя</th>
              <th class="text-right">Сумма продажы</th>
          ...
          <tbody>
            @foreach($groupedWorkers as $key => $worker)
              ...
              <tr>
                ...
                <td class="text-right">{{ number_format($worker->sum('sum'), 0, '.', ' ') . $currency }}</td>
                <td class="text-right">{{ $user->profile->discount }}%</td>
                <td class="text-right">{{ $user->profile->bonus . $currency }}</td>
